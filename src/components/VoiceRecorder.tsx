...
export const VoiceRecorder: React.FC<VoiceRecorderProps> = ({
  ...
  // Génération des ondes vocales
  const waveform = Array.from({
    length: 20
  }, (_, i) => isRecording ? Math.floor(Math.random() * 30) + 5 : 5);
  ...
  return <div className="flex flex-col items-center justify-center min-h-screen p-6 relative overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        duration: 0.3
            ...
            <motion.h2 className="text-3xl md:text-4xl font-bold text-center mb-4 bg-gradient-to-r from-bright-turquoise to-electric-blue bg-clip-text text-transparent" initial={{
          opacity: 0,
          y: -20
        }} animate={{
          opacity: 1,
          ...
          duration: 0.6
            ...
            <div className="relative mb-8">
              ...
              {/* Bouton du micro */}
              <motion.button onClick={handleMicClick} disabled={isProcessing} className={`relative z-10 w-24 h-24 rounded-full flex items-center justify-center focus:outline-none ${isRecording ? 'bg-gradient-to-r from-red-500 to-red-600' : 'bg-gradient-to-r from-bright-turquoise to-electric-blue'}`} whileHover={{
            scale: 1.05
          }} whileTap={{
            scale: 0.95
          }} animate={{
            boxShadow: isRecording ? ['0 0 0 0 rgba(239, 68, 68, 0)', '0 0 0 15px rgba(239, 68, 68, 0.3)', '0 0 0 0 rgba(239, 68, 68, 0)'] : ['0 0 0 0 rgba(0, 184, 212, 0)', '0 0 0 10px rgba(0, 184, 212, 0.3)', '0 0 0 0 rgba(0, 184, 212, 0)']
          }} transition={{
            boxShadow: {
              repeat: Infinity,
              duration: 1.5
                ...
                <motion.span className="text-white text-2xl" animate={{
            ...
            {/* Carte d'information */}
            <motion.div className="w-full max-w-md bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-slate-700" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
